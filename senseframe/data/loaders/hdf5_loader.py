...
def _find_hdf5_file(root: str, dataset_name: str) -> Path:
    ...
    try:
        ...
        raise KeyError(
            f"HDF5Loader: 数据集 '{dataset_name}' 未注册，无法派生 dir_names。"
            f"请先通过场景注册（register_dataset）声明该数据集的 dir_names。"
        )
    if not spec.dir_names:
        raise ValueError(
            f"HDF5Loader: 数据集 '{dataset_name}' 的 DatasetSpec.dir_names 为空。"
    ...
    for dir_name in spec.dir_names:
        for ext in (".h5", ".hdf5"):
            candidates.append(Path(root, dir_name, f"{dataset_name}{ext}"))
            candidates.append(Path(root, dir_name, dir_name, f"{dataset_name}{ext}"))
