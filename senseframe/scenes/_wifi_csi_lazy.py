...
首次调用任何方法时才触发 SenseFi 导入和注册，
避免 import senseframe 时因 SenseFi 缺失而报错。
"""
...
class LazyWiFiCSIContainer(SceneContainer):
    """WiFi CSI 场景延迟加载代理。

    首次调用方法时才导入 WiFiCSIContainer 并触发注册，
    之后所有调用委托给真实容器。
    """
    ...
    def _ensure(self) -> SceneContainer:
        if self._real is not None:
            return self._real
        from .wifi_csi._register import register
        register()
        from .wifi_csi.container import WiFiCSIContainer
        self._real = WiFiCSIContainer()
        return self._real
    ...
    def normalize(self, x, dataset_name: str):
        return self._ensure().normalize(x, dataset_name)
    ...
    def get_feature_spec(self, dataset_name: str, **kwargs):
        return self._ensure().get_feature_spec(dataset_name, **kwargs)

    def get_task_spec(self, dataset_name: str, model_id: str = "", **kwargs):
        return self._ensure().get_task_spec(dataset_name, model_id, **kwargs)

    def postprocess(self, model_output: Any, **kwargs) -> Any:
        return self._ensure().postprocess(model_output, **kwargs)

    def get_normalization_info(self, dataset_name: str, **kwargs):
        return self._ensure().get_normalization_info(dataset_name, **kwargs)

    def get_manifest_info(self, dataset_name: str, **kwargs):
        return self._ensure().get_manifest_info(dataset_name, **kwargs)
