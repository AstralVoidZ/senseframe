...
class LazyEEGContainer(SceneContainer):
    """EEG 场景延迟加载代理。

    首次调用方法时才导入 EEGContainer 并触发注册，
    之后所有调用委托给真实容器。
    """
    ...
    def _ensure(self) -> SceneContainer:
        if self._real is not None:
            return self._real
        from .eeg._register import register
        register()
        from .eeg.container import EEGContainer
        self._real = EEGContainer()
        return self._real
