...
class AutoAugmentSampler:
    ...
    # ============================================================
    # P3.2.5: warm_start 方法（ε4 元学习受益示例）
    # ============================================================
    def warm_start(self, source_history: List[Dict[str, Any]]) -> None:
        """P3.2.5 ε4 元学习：用源数据集成功策略作为初始 population 的种子。

        将源数据集历史中已评估的个体直接注入当前 sampler 的 population，
        ...
        Args:
            source_history: 源数据集的历史（list of dict，含 "strategy"/
                "params" 和 "result"），通常为成功策略（val_accuracy > 阈值）
        """
        for entry in source_history:
            ...
            # 优先用 val_accuracy 作为 fitness，fallback 到 value
            if isinstance(result, dict) and result.get("val_accuracy") is not None:
                fitness = float(result["val_accuracy"])
            ...
            if any(self._params_key(p) == key for p, _ in self._population):
                continue
